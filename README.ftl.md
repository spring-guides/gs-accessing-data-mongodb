...
------------------------
With your project set up, you can install and launch the Mongo database.
...
Afte you install Mongo, launch it in a console window. This command also starts up a server process. 
...
> **Note:** Mongo stores data in collections. Spring Data Mongo will map the class `Customer` into a collection called _customer_. If you want to change the name of the collection, you can use Spring Data Mongo's [`@Document`](http://static.springsource.org/spring-data/data-mongodb/docs/current/api/org/springframework/data/mongodb/core/mapping/Document.html) annotation on the class.
...
Summary
-------
Congratulations! You set up a Mongo server and wrote a simple application that uses Spring Data Mongo to save objects to and fetch them from a database -- all without writing a concrete repository implementation.